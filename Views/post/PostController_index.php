<?php if (!empty($posts)): ?>
    <div class="post-heading">
        ...
        <a href="<?= URL_ROOT ?>/post/create" class="create-post">Create Post</a>
    </div>
        ...
        <ul class="paginate__list">
            <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                <li class="paginate__item">
                    <a href="<?= URL_ROOT ?>/posts?page=<?= $i ?>" class="paginate__link <?= $current_page == $i ? "paginate__active-page" : "" ?>"><?= $i ?></a>
                </li>
            <?php endfor; ?>
        </ul>
    </div>
<?php else: ?>
    <div class="no-post-wrapper">
        <a href="<?= URL_ROOT ?>/post/create" class="create-no-post">Create Post</a>
